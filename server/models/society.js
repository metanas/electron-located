...
module.exports.getSocieties = function (page) {
    return new Promise(function (resolve, reject) {
        db.serialize(function () {
            db.all('SELECT * FROM society LIMIT ?, 20', [20 * (page - 1)], function (err, rows) {
                if (!err) {
                    resolve(rows)
                } else {
                    reject(err);
