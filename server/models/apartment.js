...
module.exports.getApartmentsFromBuilding = function (page, id) {
    return new Promise(function (resolve, reject) {
        db.serialize(function () {
            db.all('SELECT * FROM apartment where id_building=? LIMIT ?, 20', [id, (page - 1) * 20], function (err, rows) {
                if (!err) {
                    resolve(rows)
                } else {
                    reject(err)
...
module.exports.getApartments = function (page) {
    return new Promise(function (resolve, reject) {
        db.serialize(function () {
            db.all('SELECT * FROM apartment LIMIT ?, 20', [(page - 1) * 20], function (err, rows) {
                if (!err) {
                    resolve(rows)
                } else {
                    reject(err)
