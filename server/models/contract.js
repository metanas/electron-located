...
module.exports.getContracts = function (id) {
  return new Promise(function (resolve, reject) {
    var query = "SELECT c.*, cl.id as id_client, cl.lastname , cl.firstname, cl.cin, floor, area, nb_bed, description, stat, type, location_price, advance_price, status, tax, other_charge, a.id as id_apartment, a.number as nApartment, b.address as aBuilding, postcode, city, b.telephone as BTele, nb_flour, b.name as nBuilding, headquarters, s.address as aSociety ,s.telephone as STele, image, s.name as nSociety FROM contract c LEFT JOIN client as cl on (c.id_client = cl.id) LEFT JOIN apartment as a on (c.id_apartment = a.id) LEFT JOIN building as b on (a.id_building = b.id) LEFT JOIN Society as s on (s.id = b.id_society)";
    if (id) {
